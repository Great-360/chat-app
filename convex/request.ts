...
export const create = mutation({
    ...
    handler: async (ctx, args) => {
     ...
     const requestAlreadyReceived = await ctx.db.query("requests").withIndex
         ...
         ("sender", receiver._id)).first();
        if(requestAlreadyReceived) {
            throw new ConvexError("Request already received")

       }
       const friends1 = await ctx.db.query("friends").withIndex("by_user1", 
        q => q.eq("user1", currentUser._id)).collect();
     const friends2 = await ctx.db.query("friends").withIndex("by_user2",
